...
handler=get_rotating_handler("nic1.log")
logging_nic1=logging.getLogger("nic1")
logging_nic1.setLevel(logging.INFO)
logging_nic1.addHandler(handler)
...
class Run:
    ...
    def fc(self,hostname,ip,brand):
        try:
            if "." not in ip:
                with self.lock3:
                    self.result3.append((hostname,ip,brand,"ip有错误",self.time,"nic1.py"))
                logging_nic1.error(f"{hostname},{ip},{brand}。ip有错误。")
                return
            client=SSH_Server(hostname,ip,brand).client
            if client==None:
                with self.lock3:
                    self.result3.append((hostname,ip,brand,"登录不上",self.time,"nic1.py"))
                logging_nic1.error(f"{hostname},{ip},{brand}。登录不上。")
                return
            stdin,stdout,stderr=client.exec_command("esxcfg-vmknic -l | grep IPv4",timeout=60)
            output=stdout.read().decode('utf-8').strip()
            for line in output.split("\n"):
                line=line.split()
                self.result1.append((hostname,ip,brand,line[0],line[6],"虚拟",f"{line[1]}|{line[3]}"))
            ...
            with self.lock3:
                self.result3.append((hostname,ip,brand,str(e),self.time,"nic1.py"))
            logging_nic1.error(f"{hostname},{ip},{brand}。{str(e)}。")

    def collect(self):
        data=self.db_mysql.get_table_data("","select hostname,in_band_ip,brand2 from topu.server where server_group='云平台计算'")[["hostname","in_band_ip","brand2"]].values.tolist()
